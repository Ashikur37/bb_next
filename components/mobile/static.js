export const slider_mobile = {
  id: 1,
  autoplay: 1,
  autoplay_speed: 5000,
  arrows: 1,
  dots: null,
  created_at: "2019-08-06 06:34:37",
  updated_at: "2020-05-31 17:40:40",
  name: "Home Page Top Slider",
  translations: [
    {
      id: 1,
      slider_id: 1,
      locale: "en",
      name: "Home Page Top Slider",
    },
  ],
  slides: [
    {
      id: 13,
      slider_id: 1,
      options: {
        caption_1: {
          delay: null,
          effect: "fadeInUp",
        },
        caption_2: {
          delay: null,
          effect: "fadeInUp",
        },
        caption_3: {
          delay: null,
          effect: "fadeInUp",
        },
        call_to_action: {
          delay: null,
          effect: "fadeInUp",
        },
      },
      call_to_action_url: "campaigns",
      open_in_new_window: true,
      position: 1,
      created_at: "2020-07-30 19:42:17",
      updated_at: "2020-09-10 20:51:49",
      file_id: 2664,
      caption_1: null,
      caption_2: null,
      caption_3: null,
      call_to_action_text: null,
      translations: [
        {
          id: 14,
          slider_slide_id: 14,
          locale: "en",
          file_id: 2664,
          caption_1: null,
          caption_2: null,
          caption_3: null,
          call_to_action_text: null,
        },
      ],
      file: {
        id: 2664,
        user_id: 2,
        filename: "Laneige Cover-min.jpg",
        disk: "public_storage",
        path:
          "https://admin.beautyboothqa.com/storage/media/thumbnails/s74iFkN67atXT4znFHej58fjGbDzVc8n7TMpBIKH.jpeg",
        path2:
          "https://admin.beautyboothqa.com/storage/media/thumbnails/s74iFkN67atXT4znFHej58fjGbDzVc8n7TMpBIKH.jpeg",
        extension: "jpeg",
        mime: "image/jpeg",
        size: "278237",
        created_at: "2020-09-10 11:53:00",
        updated_at: "2020-09-10 11:53:00",
        thumbnail_image:
          "https://admin.beautyboothqa.com/storage/media/thumbnails/s74iFkN67atXT4znFHej58fjGbDzVc8n7TMpBIKH.jpeg",
      },
    },
    // {
    //   id: 14,
    //   slider_id: 1,
    //   options: {
    //     caption_1: {
    //       delay: null,
    //       effect: "fadeInUp",
    //     },
    //     caption_2: {
    //       delay: null,
    //       effect: "fadeInUp",
    //     },
    //     caption_3: {
    //       delay: null,
    //       effect: "fadeInUp",
    //     },
    //     call_to_action: {
    //       delay: null,
    //       effect: "fadeInUp",
    //     },
    //   },
    //   call_to_action_url: "brand/Laneige",
    //   open_in_new_window: true,
    //   position: 1,
    //   created_at: "2020-07-30 19:42:17",
    //   updated_at: "2020-09-10 20:51:49",
    //   file_id: 2664,
    //   caption_1: null,
    //   caption_2: null,
    //   caption_3: null,
    //   call_to_action_text: null,
    //   translations: [
    //     {
    //       id: 14,
    //       slider_slide_id: 14,
    //       locale: "en",
    //       file_id: 2664,
    //       caption_1: null,
    //       caption_2: null,
    //       caption_3: null,
    //       call_to_action_text: null,
    //     },
    //   ],
    //   file: {
    //     id: 2664,
    //     user_id: 2,
    //     filename: "Laneige Cover-min.jpg",
    //     disk: "public_storage",
    //     path:
    //       "https://admin.beautyboothqa.com/storage/media/aUGUk38OpLlAe6ktmri2DFxW4zuFlKiyNb2TD8ul.jpeg",
    //     path2:
    //       "https://admin.beautyboothqa.com/storage/media/aUGUk38OpLlAe6ktmri2DFxW4zuFlKiyNb2TD8ul.webp",
    //     extension: "jpeg",
    //     mime: "image/jpeg",
    //     size: "77571",
    //     created_at: "2020-08-14 16:20:07",
    //     updated_at: "2020-08-14 16:20:07",
    //     thumbnail_image:
    //       "https://admin.beautyboothqa.com/storage/media/thumbnails/aUGUk38OpLlAe6ktmri2DFxW4zuFlKiyNb2TD8ul.jpeg",
    //   },
    // },
  ],
};
export const trending_mobile = [
  [
    {
      id: 49,
      attribute_id: 8,
      slug: "toner",
      isCategory: true,
      url: "Skin-Care?sub=Toner",
      value: "Toner",
      description: null,
      translations: [
        {
          id: 45,
          attribute_value_id: 49,
          locale: "en",
          value: "Toner",
          description: null,
        },
      ],
      image: {
        id: 1,
        attribute_value_id: 49,
        image: "https://admin.beautyboothqa.com/storage/media/others/Toner.png",
        featured_image: null,
      },
    },
    {
      id: 50,
      attribute_id: 8,
      slug: "serum",
      isCategory: false,
      value: "Serum",
      description: null,
      translations: [
        {
          id: 46,
          attribute_value_id: 50,
          locale: "en",
          value: "Serum",
          description: null,
        },
      ],
      image: {
        id: 2,
        attribute_value_id: 50,
        image: "https://admin.beautyboothqa.com/storage/media/others/Serum.png",
        featured_image: null,
      },
    },
  ],
  [
    {
      id: 52,
      attribute_id: 8,
      slug: "moisturizer",
      isCategory: false,
      value: "Moisturizer",
      description: null,
      translations: [
        {
          id: 48,
          attribute_value_id: 52,
          locale: "en",
          value: "Moisturizer",
          description: null,
        },
      ],
      image: {
        id: 3,
        attribute_value_id: 52,
        image:
          "https://admin.beautyboothqa.com/storage/media/others/Moisturizer.png",
        featured_image: null,
      },
    },
    {
      id: 53,
      attribute_id: 8,
      slug: "bundle-set",
      isCategory: true,
      url: "Skin-Care?sub=Bundle-Set",
      value: "Bundle Set",
      description: null,
      translations: [
        {
          id: 49,
          attribute_value_id: 53,
          locale: "en",
          value: "Bundle Set",
          description: null,
        },
      ],
      image: {
        id: 4,
        attribute_value_id: 53,
        image:
          "https://admin.beautyboothqa.com/storage/media/others/Bundle Set.png",
        featured_image: null,
      },
    },
  ],
  [
    {
      id: 55,
      attribute_id: 8,
      slug: "disinfector",
      isCategory: false,
      value: "Disinfector",
      description: null,
      translations: [
        {
          id: 51,
          attribute_value_id: 55,
          locale: "en",
          value: "Disinfector",
          description: null,
        },
      ],
      image: {
        id: 5,
        attribute_value_id: 55,
        image:
          "https://admin.beautyboothqa.com/storage/media/others/Disinfection.png",
        featured_image: null,
      },
    },
    {
      id: 56,
      attribute_id: 8,
      slug: "lip-tint",
      isCategory: false,
      value: "Lip Tint",
      description: null,
      translations: [
        {
          id: 52,
          attribute_value_id: 56,
          locale: "en",
          value: "Lip Tint",
          description: null,
        },
      ],
      image: {
        id: 6,
        attribute_value_id: 56,
        image:
          "https://admin.beautyboothqa.com/storage/media/others/Lip TInt.png",
        featured_image: null,
      },
    },
  ],
  [
    {
      id: 57,
      attribute_id: 8,
      slug: "face-mask",
      isCategory: false,
      value: "Face Mask",
      description: null,
      translations: [
        {
          id: 53,
          attribute_value_id: 57,
          locale: "en",
          value: "Face Mask",
          description: null,
        },
      ],
      image: {
        id: 7,
        attribute_value_id: 57,
        image:
          "https://admin.beautyboothqa.com/storage/media/others/Face mask.png",
        featured_image: null,
      },
    },
    {
      id: 58,
      attribute_id: 8,
      slug: "hair-care",
      isCategory: false,
      value: "Hair Care",
      description: null,
      translations: [
        {
          id: 54,
          attribute_value_id: 58,
          locale: "en",
          value: "Hair Care",
          description: null,
        },
      ],
      image: {
        id: 8,
        attribute_value_id: 58,
        image:
          "https://admin.beautyboothqa.com/storage/media/others/Hair Wash.png",
        featured_image: null,
      },
    },
  ],
  [
    {
      id: 59,
      attribute_id: 8,
      slug: "sun-protection",
      isCategory: false,
      value: "Sun Protection",
      description: null,
      translations: [
        {
          id: 55,
          attribute_value_id: 59,
          locale: "en",
          value: "Sun Protection",
          description: null,
        },
      ],
      image: {
        id: 9,
        attribute_value_id: 59,
        image:
          "https://admin.beautyboothqa.com/storage/media/others/Sun Proc.png",
        featured_image: null,
      },
    },
    {
      id: 60,
      attribute_id: 8,
      slug: "cushion",
      isCategory: true,
      url: "Makeup?sub=Cushion",
      value: "Cushion",
      description: null,
      translations: [
        {
          id: 56,
          attribute_value_id: 60,
          locale: "en",
          value: "Cushion",
          description: null,
        },
      ],
      image: {
        id: 10,
        attribute_value_id: 60,
        image:
          "https://admin.beautyboothqa.com/storage/media/others/Make up.png",
        featured_image: null,
      },
    },
  ],
  [
    {
      id: 61,
      attribute_id: 8,
      slug: "body-care",
      isCategory: true,
      value: "Body Care",
      url: "Body-Care",
      description: null,
      translations: [
        {
          id: 57,
          attribute_value_id: 61,
          locale: "en",
          value: "Body Care",
          description: null,
        },
      ],
      image: {
        id: 11,
        attribute_value_id: 61,
        image:
          "https://admin.beautyboothqa.com/storage/media/others/Body Wash.png",
        featured_image: null,
      },
    },
    {
      id: 62,
      attribute_id: 8,
      slug: "anti-aging",
      isCategory: false,
      value: "Anti-Aging",
      description: null,
      translations: [
        {
          id: 58,
          attribute_value_id: 62,
          locale: "en",
          value: "Anti-Aging",
          description: null,
        },
      ],
      image: {
        id: 12,
        attribute_value_id: 62,
        image:
          "https://admin.beautyboothqa.com/storage/media/others/Anti Aging.png",
        featured_image: null,
      },
    },
  ],
  [
    {
      id: 63,
      attribute_id: 8,
      slug: "eye-shadow",
      isCategory: false,
      value: "Eye Shadow",
      description: null,
      translations: [
        {
          id: 59,
          attribute_value_id: 63,
          locale: "en",
          value: "Eye Shadow",
          description: null,
        },
      ],
      image: {
        id: 13,
        attribute_value_id: 63,
        image:
          "https://admin.beautyboothqa.com/storage/media/others/Eye Shadow.png",
        featured_image: null,
      },
    },
    {
      id: 64,
      attribute_id: 8,
      slug: "cleanser",
      isCategory: false,
      value: "Cleanser",
      description: null,
      translations: [
        {
          id: 60,
          attribute_value_id: 64,
          locale: "en",
          value: "Cleanser",
          description: null,
        },
      ],
      image: {
        id: 14,
        attribute_value_id: 64,
        image:
          "https://admin.beautyboothqa.com/storage/media/others/Cleaner.png",
        featured_image: null,
      },
    },
  ],
  [
    {
      id: 67,
      attribute_id: 8,
      slug: "kit",
      isCategory: false,
      value: "Kit",
      description: null,
      translations: [
        {
          id: 63,
          attribute_value_id: 67,
          locale: "en",
          value: "Kit",
          description: null,
        },
      ],
      image: {
        id: 15,
        attribute_value_id: 67,
        image: "https://admin.beautyboothqa.com/storage/media/others/Kit.png",
        featured_image: null,
      },
    },
    {
      id: 73,
      attribute_id: 8,
      slug: "face-roller",
      isCategory: false,
      value: "Face Roller",
      description: null,
      translations: [
        {
          id: 69,
          attribute_value_id: 73,
          locale: "en",
          value: "Face Roller",
          description: null,
        },
      ],
      image: {
        id: 16,
        attribute_value_id: 73,
        image:
          "https://admin.beautyboothqa.com/storage/media/others/Roller.png",
        featured_image: null,
      },
    },
  ],
];
export const concern_mobile = [
  {
    id: 80,
    attribute_id: 7,
    slug: "acne-solution",
    value: "Acne Solution",
    description: null,
    translations: [
      {
        id: 76,
        attribute_value_id: 80,
        locale: "en",
        value: "Acne Solution",
        description: null,
      },
    ],
    image: {
      id: 17,
      attribute_value_id: 80,
      image: "https://admin.beautyboothqa.com/storage/media/others/Acne.png",
      featured_image: null,
    },
  },
  {
    id: 29,
    attribute_id: 7,
    slug: "anti-aging",
    value: "Anti Aging",
    description: null,
    translations: [
      {
        id: 26,
        attribute_value_id: 29,
        locale: "en",
        value: "Anti Aging",
        description: null,
      },
    ],
    image: {
      id: 18,
      attribute_value_id: 29,
      image:
        "https://admin.beautyboothqa.com/storage/media/others/Anti aging.png",
      featured_image: null,
    },
  },
  {
    id: 81,
    attribute_id: 7,
    slug: "dry-skin",
    value: "Dry Skin",
    description: null,
    translations: [
      {
        id: 77,
        attribute_value_id: 81,
        locale: "en",
        value: "Dry Skin",
        description: null,
      },
    ],
    image: {
      id: 19,
      attribute_value_id: 81,
      image:
        "https://admin.beautyboothqa.com/storage/media/others/Dry Skin.png",
      featured_image: null,
    },
  },
  {
    id: 30,
    attribute_id: 7,
    slug: "wrinkles",
    value: "Wrinkles",
    description: null,
    translations: [
      {
        id: 27,
        attribute_value_id: 30,
        locale: "en",
        value: "Wrinkles",
        description: null,
      },
    ],
    image: {
      id: 23,
      attribute_value_id: 30,
      image: "https://admin.beautyboothqa.com/storage/media/others/Wrinkle.png",
      featured_image: null,
    },
  },
  {
    id: 82,
    attribute_id: 7,
    slug: "pore-care",
    value: "Pore Care",
    description: null,
    translations: [
      {
        id: 78,
        attribute_value_id: 82,
        locale: "en",
        value: "Pore Care",
        description: null,
      },
    ],
    image: {
      id: 20,
      attribute_value_id: 82,
      image:
        "https://admin.beautyboothqa.com/storage/media/others/Pore skin.png",
      featured_image: null,
    },
  },
  // {
  //   id: 83,
  //   attribute_id: 7,
  //   slug: "brightening",
  //   value: "Brightening",
  //   description: null,
  //   translations: [
  //     {
  //       id: 79,
  //       attribute_value_id: 83,
  //       locale: "en",
  //       value: "Brightening",
  //       description: null,
  //     },
  //   ],
  //   image: {
  //     id: 21,
  //     attribute_value_id: 83,
  //     image:
  //       "https://admin.beautyboothqa.com/storage/media/others/white brighte.png",
  //     featured_image: null,
  //   },
  // },
  {
    id: 84,
    attribute_id: 7,
    slug: "sensitive-skin",
    value: "Sensitive Skin",
    description: null,
    translations: [
      {
        id: 80,
        attribute_value_id: 84,
        locale: "en",
        value: "Sensitive Skin",
        description: null,
      },
    ],
    image: {
      id: 22,
      attribute_value_id: 84,
      image:
        "https://admin.beautyboothqa.com/storage/media/others/sensi skin.png",
      featured_image: null,
    },
  },
];

export const cityNames = [
  "Al Adhbah",
  "Al Daayen",
  "Al Hitmi",
  "Al Jasrah",
  "Al Jumaliyah",
  "Al Ka'biyah",
  "Al Khalifat",
  "Al Khawr",
  "Al Khor",
  "Al Khuwayr",
  "Al Mafjar",
  "Al Qaabiyah",
  "Al Rayyan",
  "Al Sailiya",
  "Al Shamal",
  "Al Wakhra",
  "Doha",
  "Dukhan",
  "Gharafa",
  "Ras Laffan",
  "Shahaniya",
  "Umm Bab",
  "Umm Sa'id",
  "Umm Salal ALi",
  "Umm Salal Muhammad",
];
const cloud = "https://beautyboothqa.sgp1.cdn.digitaloceanspaces.com/assets/mobile/";
export const menus = [
  {
    hasItems: false,
    isParent: true,
    isCategory: false,
    slug: "new",
    name: "New",
    image:
      { "en": cloud+"menu/en/new.jpg", "ar": cloud+"menu/ar_QA/new.jpg" },
    items: [],
  },
  {
    hasItems: true,
    isParent: true,
    isCategory: false,
    slug: "brands",
    name: "BRANDS",
    image:
    { "en": cloud+"menu/en/brands.jpg", "ar": cloud+"menu/ar_QA/brands.jpg" },
    items: [
      {
        hasItems: false,
        isCategory: false,
        isParent: false,
        slug: "/brand/Nature Republic",
        name: "Nature Republic",
      },
      {
        hasItems: false,
        isCategory: false,
        isParent: false,
        slug: "/brand/Cathy Doll",
        name: "Cathy Doll",
      },
      {
        hasItems: false,
        isCategory: false,
        isParent: false,
        slug: "/brand/Baby Bright",
        name: "Baby Bright",
      },
      {
        hasItems: false,
        isCategory: false,
        isParent: false,
        slug: "/brand/Innisfree",
        name: "Innisfree",
      },
      {
        hasItems: false,
        isCategory: false,
        isParent: false,
        slug: "/brand/Holika Holika",
        name: "Holika Holika",
      },
      {
        hasItems: false,
        isCategory: false,
        isParent: false,
        slug: "/brand/Cosrx",
        name: "Cosrx",
      },
      {
        hasItems: false,
        isCategory: false,
        isParent: false,
        slug: "/brand/The Ordinary",
        name: "The Ordinary",
      },
      {
        hasItems: false,
        isCategory: false,
        isParent: false,
        slug: "/brand/Some By Mi",
        name: "Some By Mi",
      },
      {
        hasItems: false,
        isCategory: false,
        isParent: false,
        slug: "/brand/Etude House",
        name: "Etude House",
      },
      {
        hasItems: false,
        isCategory: false,
        isParent: false,
        slug: "/brand/Laneige",
        name: "Laneige",
      },
      {
        hasItems: false,
        isCategory: false,
        isParent: false,
        slug: "/brand/The Face Shop",
        name: "The Face Shop",
      },
      {
        hasItems: false,
        isCategory: false,
        isParent: false,
        slug: "/brand/Tiam",
        name: "Tiam",
      },
      {
        hasItems: false,
        isCategory: false,
        isParent: false,
        slug: "/brand/Mizon",
        name: "Mizon",
      },
      {
        hasItems: false,
        isCategory: false,
        isParent: false,
        slug: "/brand/Missha",
        name: "Missha",
      },
      {
        hasItems: false,
        isCategory: false,
        isParent: false,
        slug: "/brand/Tosowoong",
        name: "Tosowoong",
      },
      {
        hasItems: false,
        isCategory: false,
        isParent: false,
        slug: "/brand/April Skin",
        name: "April Skin",
      },
      {
        hasItems: false,
        isCategory: false,
        isParent: false,
        slug: "/brand/Secret Key",
        name: "Secret Key",
      },
      {
        hasItems: false,
        isCategory: false,
        isParent: false,
        slug: "/brand/Tonymoly",
        name: "Tonymoly",
      },
      {
        hasItems: false,
        isCategory: false,
        isParent: false,
        slug: "/brand/Benton",
        name: "Benton",
      },
      {
        hasItems: false,
        isCategory: false,
        isParent: false,
        slug: "/brand/Peripera",
        name: "Peripera",
      },
      {
        hasItems: false,
        isCategory: false,
        isParent: false,
        slug: "/brand/Skinfood",
        name: "Skinfood",
      },
      {
        hasItems: false,
        isCategory: false,
        isParent: false,
        slug: "/brand/Nacific",
        name: "Nacific",
      },
      {
        hasItems: false,
        isCategory: false,
        isParent: false,
        slug: "/brand/Goli",
        name: "Goli",
      },
      {
        hasItems: false,
        isCategory: false,
        isParent: false,
        slug: "/brand/Reunrom",
        name: "Reunrom",
      },
      {
        hasItems: false,
        isCategory: false,
        isParent: false,
        slug: "/brand/Purito",
        name: "Purito",
      },
      {
        hasItems: false,
        isCategory: false,
        isParent: false,
        slug: "/brand/Banila Co",
        name: "Banila Co",
      },
      {
        hasItems: false,
        isCategory: false,
        isParent: false,
        slug: "/brand/Skynlab",
        name: "Skynlab",
      },
      {
        hasItems: false,
        isCategory: false,
        isParent: false,
        slug: "/brand/Dr.g",
        name: "Dr.g",
      },
      {
        hasItems: false,
        isCategory: false,
        isParent: false,
        slug: "/brand/Ma:nyo",
        name: "Ma:nyo",
      },
      {
        hasItems: false,
        isCategory: false,
        isParent: false,
        slug: "/brand/Cnp Laboratory",
        name: "Cnp Laboratory",
      },
      {
        hasItems: false,
        isCategory: false,
        isParent: false,
        slug: "/brand/I Love Me",
        name: "I Love Me",
      },
      {
        hasItems: false,
        isCategory: false,
        isParent: false,
        slug: "/brand/Neogen",
        name: "Neogen",
      },
      {
        hasItems: false,
        isCategory: false,
        isParent: false,
        slug: "/brand/Pyunkang Yul",
        name: "Pyunkang Yul",
      },
      {
        hasItems: false,
        isCategory: false,
        isParent: false,
        slug: "/brand/Dear Klairs",
        name: "Dear Klairs",
      },
      {
        hasItems: false,
        isCategory: false,
        isParent: false,
        slug: "/brand/Papa Recipe",
        name: "Papa Recipe",
      },
      {
        hasItems: false,
        isCategory: false,
        isParent: false,
        slug: "/brand/Blink",
        name: "Blink",
      },
    ],
  },
  {
    hasItems: true,
    isParent: true,
    isCategory: true,
    slug: "skin-care",
    name: "Skin Care",
    url: "Skin-Care",
    image:
    { "en": cloud+"menu/en/skincare.jpg", "ar": cloud+"menu/ar_QA/skincare.jpg" },
    items: [
      {
        hasItems: false,
        isParent: false,
        isCategory: true,
        name: "ampoule",
        url: "Skin-Care?sub=ampoule",
        slug: "ampoule",
        // image: "Serum.jpg",
        image: "skin-care/Ampoule.png",
        items: [],
      },
      {
        hasItems: false,
        isParent: false,
        isCategory: true,
        slug: "bundle-set",
        // image: "Bundle.jpg",
        image: "skin-care/Bundle.png",
        items: [],
        name: "Bundle Set",
        url: "Skin-Care?sub=Bundle-Set",
      },
      {
        hasItems: true,
        isParent: false,
        isCategory: true,
        slug: "cleansers",
        // image: "Cleaner.jpg",
        image: "skin-care/Cleaner.png",
        items: [
          {
            hasItems: false,
            isParent: false,
            isCategory: true,
            slug: "cleansing-balms",
            url: "Skin-Care?sub=Cleansers&child=Cleansing-Balms",
            image: "cleansing_balm.jpg",
            items: [],
            name: "Cleansing Balms",
          },
          {
            hasItems: false,
            isParent: false,
            isCategory: true,
            slug: "cleansing-gels",
            image: "Gel.jpg",
            items: [],
            name: "Cleansing Gels",
            url: "Skin-Care?sub=Cleansers&child=Cleansing-Gels",
          },
          {
            hasItems: false,
            isParent: false,
            isCategory: true,
            slug: "cleansing-oils",
            image: "Oil.jpg",
            items: [],
            name: "Cleansing Oils",
            url: "Skin-Care?sub=Cleansers&child=Cleansing-Oils",
          },
          {
            hasItems: false,
            isParent: false,
            isCategory: true,
            slug: "cleansing-waters",
            image: "Water.jpg",
            items: [],
            name: "Cleansing Waters",
            url: "Skin-Care?sub=Cleansers&child=Cleansing-Waters",
          },
          {
            hasItems: false,
            isParent: false,
            isCategory: true,
            slug: "face-wipes",
            image: "face_wipes.jpg",
            items: [],
            name: "Face Wipes",
            url: "Skin-Care?sub=Cleansers&child=Face-Wipes",
          },
          {
            hasItems: false,
            isParent: false,
            isCategory: true,
            slug: "facial-cleansing-bars",
            image: "soap_bars.jpg",
            items: [],
            name: "Facial Cleansing bars",
            url: "Skin-Care?sub=Cleansers&child=Facial-Cleansing-bars",
          },
          {
            hasItems: false,
            isParent: false,
            isCategory: true,
            slug: "foaming-cleansers",
            image: "foam.jpg",
            items: [],
            name: "Foaming Cleansers",
            url: "Skin-Care?sub=Cleansers&child=Foaming-Cleansers",
          },
          {
            hasItems: false,
            isParent: false,
            isCategory: true,
            slug: "make-up-removers",
            image: "make_up_removal.jpg",
            items: [],
            name: "Make Up Removers",
            url: "Skin-Care?sub=Cleansers&child=Make-Up-Removers",
          },
        ],
        name: "Cleansers",
        url: "Skin-Care?sub=Cleansers",
      },
      {
        hasItems: false,
        isParent: false,
        isCategory: true,
        slug: "cream",
        // image: "Cream.jpg",
        image: "skin-care/Cream.png",
        items: [],
        name: "Cream",
        url: "Skin-Care?sub=Cream",
      },
      {
        hasItems: false,
        isParent: false,
        isCategory: true,
        slug: "essence",
        // image: "Essence.jpg",
        image: "skin-care/Essence.png",
        items: [],
        name: "Essence",
        url: "Skin-Care?sub=Essence",
      },
      {
        hasItems: true,
        isParent: false,
        isCategory: true,
        slug: "eye",
        name: "Eye",
        url: "Skin-Care?sub=Eye",
        // image: "Eye.jpg",
        image: "skin-care/Eye.png",
        items: [
          {
            hasItems: false,
            isParent: false,
            isCategory: true,
            slug: "eye-cream",
            image: "eye_cream.jpg",
            items: [],
            name: "Eye Cream",
            url: "Skin-Care?sub=Eye&child=Eye-Cream",
          },
          {
            hasItems: false,
            isParent: false,
            isCategory: true,
            slug: "eye-patch",
            image: "eye_patch.jpg",
            items: [],
            name: "Eye Patch",
            url: "Skin-Care?sub=Eye&child=Eye-Patch",
          },
          {
            hasItems: false,
            isParent: false,
            isCategory: true,
            slug: "eye-serum",
            image: "eye_cream.jpg",
            items: [],
            name: "Eye Serum",
            url: "Skin-Care?sub=Eye&child=Eye-Serum",
          },
        ],
        name: "Eye",
      },
      {
        hasItems: false,
        isParent: false,
        isCategory: true,
        slug: "face-roller",
        // image: "Roller.jpg",
        image: "skin-care/Roller.png",
        items: [],
        name: "Face Roller",
        url: "Skin-Care?sub=Face-Roller",
      },
      {
        hasItems: false,
        isParent: false,
        isCategory: true,
        slug: "kit",
        // image: "kIT.jpg",
        image: "skin-care/Kit.png",
        items: [],
        name: "Kit",
        url: "Skin-Care?sub=Kit",
      },
      {
        hasItems: true,
        isParent: false,
        isCategory: true,
        slug: "lip-care",
        // image: "Lipsticks.jpg",
        image: "skin-care/Lipsticks.png",
        items: [
          {
            hasItems: false,
            isParent: false,
            isCategory: true,
            slug: "lip-balm",
            image: null,
            items: [],
            name: "Lip Balm",
            url: "Skin-Care?sub=Lip-Care&child=Lip-Balm",
          },
          {
            hasItems: false,
            isParent: false,
            isCategory: true,
            slug: "lip-sleeping-mask",
            image: null,
            items: [],
            name: "Lip Sleeping Mask",
            url: "Skin-Care?sub=Lip-Care&child=Lip-Sleeping-Mask",
          },
        ],
        name: "Lip Care",
        url: "Skin-Care?sub=Lip-Care",
      },
      {
        hasItems: true,
        isParent: false,
        isCategory: true,
        slug: "masks-exfolators",
        // image: "Mask.jpg",
        image: "skin-care/Mask.png",
        items: [
          {
            hasItems: false,
            isParent: false,
            isCategory: true,
            slug: "exfoliators",
            image: "EXFOLATORS.jpg",
            items: [],
            name: "Exfoliators",
            url: "Skin-Care?sub=Masks-&-Exfolators&child=Exfoliators",
          },
          {
            hasItems: false,
            isParent: false,
            isCategory: true,
            slug: "mask-sheet",
            image: "mask_sheet.jpg",
            items: [],
            name: "Mask Sheet",
            url: "Skin-Care?sub=Masks-%26-Exfolators&child=Mask-Sheet",
          },
          {
            hasItems: false,
            isParent: false,
            isCategory: true,
            slug: "patchs",
            image: "Patchs.jpg",
            items: [],
            name: "Patchs",
            url: "Skin-Care?sub=Masks-%26-Exfolators&child=Patchs",
          },
          {
            hasItems: false,
            isParent: false,
            isCategory: true,
            slug: "wash-off-mask",
            image: "mask_off.jpg",
            items: [],
            name: "Wash Off Mask",
            url: "Skin-Care?sub=Masks-%26-Exfolators&child=Wash-Off-Mask",
          },
        ],
        name: "Masks & Exfolators",
        url: "Skin-Care?sub=Masks-%26-Exfolators",
      },
      {
        hasItems: false,
        isParent: false,
        isCategory: true,
        slug: "scrub",
        // image: "Scrub.jpg",
        image: "skin-care/Scrub.png",
        items: [],
        name: "Scrub",
        url: "Skin-Care?sub=Scrub",
      },
      {
        hasItems: false,
        isParent: false,
        isCategory: true,
        slug: "serum",
        // image: "Serum.jpg",
        image: "skin-care/Serum.png",
        items: [],
        name: "Serum",
        url: "Skin-Care?sub=Serum",
      },
      {
        hasItems: false,
        isParent: false,
        isCategory: true,
        slug: "soothing-gel",
        // image: "Soothing_gel.jpg",
        image: "skin-care/Soothing_gel.png",
        items: [],
        name: "Soothing Gel",
        url: "Skin-Care?sub=Soothing-Gel",
      },
      {
        hasItems: false,
        isParent: false,
        isCategory: true,
        slug: "sun-care",
        // image: "Sun_cream.jpg",
        image: "skin-care/Sun_cream.png",
        items: [],
        name: "Sun Care",
        url: "Skin-Care?sub=Sun-Care",
      },
      {
        hasItems: false,
        isParent: false,
        isCategory: true,
        slug: "toner",
        // image: "Toner.png",
        image: "skin-care/Toner.png",
        items: [],
        name: "Toner",
        url: "Skin-Care?sub=Toner",
      },
    ],
  },
  {
    hasItems: true,
    isParent: true,
    isCategory: true,
    slug: "body-care",
    name: "Body Care",
    url: "Body-Care",
    image:
    { "en": cloud+"menu/en/bodycare.jpg", "ar": cloud+"menu/ar_QA/bodycare.jpg" },
    items: [
      {
        hasItems: false,
        isParent: false,
        isCategory: true,
        slug: "bath",
        image: "body-care/bath.png",
        items: [],
        name: "Bath",
        url: "Body-Care?sub=Bath",
      },
      {
        hasItems: false,
        isParent: false,
        isCategory: true,
        slug: "bb-body-cream",
        image: "body-care/bb-body-cream.png",
        items: [],
        name: "BB Body Cream",
        url: "Body-Care?sub=BB-Body-Cream",
      },
      {
        hasItems: false,
        isParent: false,
        isCategory: true,
        slug: "body-sun-care",
        image: "body-care/body_sun.png",
        items: [],
        name: "Body Sun Care",
        url: "Body-Care?sub=Body-Sun-Care",
      },
      {
        hasItems: false,
        isParent: false,
        isCategory: true,
        image: "body-care/Cream.png",
        items: [],
        name: "Lotion",
        url: "Body-Care?sub=Lotion",
        slug: "lotion",
      },
      {
        hasItems: true,
        isParent: false,
        isCategory: true,
        slug: "hand-foot-care",
        image: "body-care/7.png",
        items: [
          {
            hasItems: false,
            isParent: false,
            isCategory: true,
            slug: "foot-mask",
            image: "foot.jpg",
            items: [],
            name: "Foot Mask",
            url: "Body-Care?sub=Hand-%26-Foot-Care%2FFoot-Mask",
          },
          {
            hasItems: false,
            isParent: false,
            isCategory: true,
            slug: "hand-cream",
            image: "9.jpg",
            items: [],
            name: "Hand Cream",
            url: "Body-Care?sub=Hand-%26-Foot-Care%2FHand-Cream",
          },
          {
            hasItems: false,
            isParent: false,
            isCategory: true,
            slug: "hand-mask",
            image: "8.jpg",
            items: [],
            name: "Hand Mask",
            url: "Body-Care?sub=Hand-%26-Foot-Care%2FHand-Mask",
          },
          {
            hasItems: false,
            isParent: false,
            isCategory: true,
            slug: "hand-sanitizer",
            image: "10.jpg",
            items: [],
            name: "Hand Sanitizer",
            url: "Body-Care?sub=Hand-%26-Foot-Care%2FHand-Sanitizer",
          },
        ],
        name: "Hand & Foot Care",
        url: "Body-Care?sub=Hand-%26-Foot-Care",
      },
      {
        hasItems: false,
        isParent: false,
        isCategory: true,
        slug: "intimate-care",
        image: "body-care/intimate_care.png",
        items: [],
        name: "Intimate Care",
        url: "Body-Care?sub=Intimate-Care",
      },
      // {
      //   hasItems: false,
      //   isParent: false,
      //   isCategory: true,
      //   slug: "supplement",
      //   image: "Essence.jpg",
      //   items: [],
      //   name: "Supplement",
      //   url: "Body-Care?sub=Supplement",
      // },
    ],
  },
  {
    hasItems: true,
    isParent: true,
    isCategory: true,
    slug: "hair-care",
    name: "Hair Care",
    url: "Hair-Care",
    image:
    { "en": cloud+"menu/en/haricare.jpg", "ar": cloud+"menu/ar_QA/haricare.jpg" },
    items: [
      {
        hasItems: false,
        isParent: false,
        isCategory: true,
        slug: "conditioner",
        image: "hair-care/Conditioner.png",
        items: [],
        name: "Conditioner",
        url: "Hair-Care?sub=Conditioner",
      },
      {
        hasItems: false,
        isParent: false,
        isCategory: true,
        slug: "hair-color",
        image: "hair-care/hair_color.png",
        items: [],
        name: "Hair Color",
        url: "Hair-Care?sub=Hair-Color",
      },
      {
        hasItems: false,
        isParent: false,
        isCategory: true,
        slug: "hair-cream-masks",
        image: "hair-care/hair_cream.png",
        items: [],
        name: "Hair Cream & Masks",
        url: "Hair-Care?sub=Hair-Cream-%26-Masks",
      },
      {
        hasItems: false,
        isParent: false,
        isCategory: true,
        slug: "hair-serum",
        image: "hair-care/hair_serum.png",
        items: [],
        name: "Hair Serum",
        url: "Hair-Care?sub=Hair-Serum",
      },
      {
        hasItems: false,
        isParent: false,
        isCategory: true,
        slug: "shampoo",
        image: "hair-care/shampoo.png",
        items: [],
        name: "Shampoo",
        url: "Hair-Care?sub=Shampoo",
      },
      {
        hasItems: false,
        isParent: false,
        isCategory: true,
        slug: "treatment",
        image: "hair-care/treatment.png",
        items: [],
        name: "Treatment",
        url: "Hair-Care?sub=Treatment",
      },
    ],
  },
  {
    hasItems: true,
    isParent: true,
    isCategory: true,
    slug: "makeup",
    name: "Makeup",
    url: "Makeup",
    image:
    { "en": cloud+"menu/en/makeup.jpg", "ar": cloud+"menu/ar_QA/makeup.jpg" },
    items: [
      {
        hasItems: false,
        isParent: false,
        isCategory: true,
        slug: "base-makeup",
        image: "make-up/base make up.png",
        items: [],
        name: "Base Makeup",
        url: "Makeup?sub=Base-Makeup",
      },
      {
        hasItems: false,
        isParent: false,
        isCategory: true,
        slug: "beauty-accessories",
        image: "make-up/accesso.png",
        items: [],
        name: "Beauty Accessories",
        url: "Makeup?sub=Beauty-Accessories",
      },
      {
        hasItems: false,
        isParent: false,
        isCategory: true,
        image: "make-up/base make up.png",
        items: [],
        name: "Blusher",
        url: "Makeup?sub=Blusher",
        slug: "blusher",
      },
      {
        hasItems: false,
        isParent: false,
        isCategory: true,
        slug: "cushion",
        image: "make-up/Cushion.png",
        items: [],
        name: "Cushion",
        url: "Makeup?sub=Cushion",
      },
      {
        hasItems: true,
        isParent: false,
        isCategory: true,
        slug: "eye",
        image: "make-up/Eyes.png",
        items: [
          {
            hasItems: false,
            isParent: false,
            isCategory: true,
            slug: "eye-liner",
            image: "Eye liner.jpg",
            items: [],
            name: "Eye Liner",
            url: "Makeup?sub=Eye&child=Eye-Liner",
          },
          {
            hasItems: false,
            isParent: false,
            isCategory: true,
            slug: "eye-shadow",
            image: "Eye shadow.jpg",
            items: [],
            name: "Eye Shadow",
            url: "Makeup?sub=Eye&child=Eye-Shadow",
          },
          {
            hasItems: false,
            isParent: false,
            isCategory: true,
            slug: "eyebrow",
            image: "eye brow.jpg",
            items: [],
            name: "EyeBrow",
            url: "Makeup?sub=Eye&child=EyeBrow",
          },
          {
            hasItems: false,
            isParent: false,
            isCategory: true,
            slug: "mascara",
            image: "mascara.jpg",
            items: [],
            name: "Mascara",
            url: "Makeup?sub=Eye&child=Mascara",
          },
        ],
        name: "Eye",
        url: "Makeup?sub=Eye",
      },
      {
        hasItems: false,
        isParent: false,
        isCategory: true,
        image: "make-up/highlighter.png",
        items: [],
        name: "Highlighter",
        url: "Makeup?sub=Highlighter",
        slug: "highlighter",
      },
      {
        hasItems: true,
        isParent: false,
        isCategory: true,
        slug: "lip",
        image: "make-up/Lip.png",
        items: [
          {
            hasItems: false,
            isParent: false,
            isCategory: true,
            slug: "lip-tint",
            image: "Lip tint.jpg",
            items: [],
            name: "Lip Tint",
            url: "Makeup?sub=Lip&child=Lip-Tint",
          },
          {
            hasItems: false,
            isParent: false,
            isCategory: true,
            slug: "lipstick",
            image: "lipstick.jpg",
            items: [],
            name: "lipstick",
            url: "Makeup?sub=Lip&child=lipstick",
          },
        ],
        name: "Lip",
        url: "Makeup?sub=Lip",
      },
    ],
  },
  {
    hasItems: false,
    isParent: true,
    isCategory: true,
    slug: "fragrance",
    name: "Fragrance",
    url: "Fragrance",
    image:
    { "en": cloud+"menu/en/fragrance.jpg", "ar": cloud+"menu/ar_QA/fragrance.jpg" },
    items: [],
  },
  {
    hasItems: true,
    isParent: true,
    isCategory: true,
    slug: "accessories",
    name: "Accessories",
    url: "Accessories",
    image:
    { "en": cloud+"menu/en/accessoreis.jpg", "ar": cloud+"menu/ar_QA/accessoreis.jpg" },
    items: [
      {
        hasItems: false,
        isParent: false,
        isCategory: true,
        name: "Beauty Tools",
        url: "Accessories?sub=Beauty-Tools",
        slug: "beauty-tools",
        items: [],
      },
      {
        hasItems: false,
        isParent: false,
        isCategory: true,
        name: "Women's Fashion Jewelry",
        url: "Accessories?sub=Women's-Fashion-Jewelry",
        slug: "womens-fashion-jewelry-accessories",
        items: [],
      },
    ],
  },
  {
    hasItems: false,
    isParent: true,
    isCategory: false,
    slug: "campaigns",
    name: "Campaigns",
    url: "Campaigns",
    image:
    { "en": cloud+"menu/en/campaign.jpg", "ar": cloud+"menu/ar_QA/campaign.jpg" },
    items: [],
  },
  {
    hasItems: false,
    isParent: true,
    isCategory: false,
    slug: "best-selling",
    name: "Best Selling",
    image:
    { "en": cloud+"menu/en/bestselling.jpg", "ar": cloud+"menu/ar_QA/bestselling.jpg" },
    items: [],
  },
];
