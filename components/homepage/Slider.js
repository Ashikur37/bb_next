...
const Slider = ({ styles, lang, slides, isMobile, Link }) => {
            ...
            "slides": [
              {
                ...
                "file": {
                  ...
                  "thumbnail_image": "https://admin.beautyboothqa.com/storage/media/thumbnails/PcKe7LT5dInKosuoFjpCnw5bXuPhk141SYIRiSiD.jpeg"
                }
              },
              {
                "id": 21,
    ...
    return (
        <div className={`container-fluid`}>
            <div className={`row align-self-center ${styles.sliderContainer}`}>
                {isMobile ?
                    <>
                        {slider ? (
                            <Swiper {...params}>
                                {slider.map((slide, index) => (
                                    <div className="swiper_banner_container" key={index}>
                                        {index == 0 && <link rel="preload" as="image" href={slide.imagePath} />}
                                        <Link href={slide.url ?? "/"}>
                                            <a>
                        ...
                        {slider ? (
                            <Swiper {...params}>
                                {slider.slides.map((slide, index) => (
                                    <div className="swiper_banner_container" key={index}>
                                        {index == 0 && <link rel="preload" as="image" href={slide.file.path} />}
                                        <Link href={slide.call_to_action_url ?? "/"}>
                                            <a>
                                                <img src={slide.file.path} alt={slide.file.filename} />
                                            </a>
                                        </Link>
                                    </div>
                                ))}
